...
/// <summary>
///  Represents bit flags that specify the file attributes of the link target, if the target is a file system item.
///  File attributes can be used if the link target is not available, or if accessing the target would be inefficient.
///  It is possible for the target items attributes to be out of sync with this value.
/// </summary>
[Flags]
public enum FileAttributesFlags : int
{
    None = 0,
    /// <summary>
    /// The file or directory is read-only.
    /// For a file, if this bit is set, applications can read the file but cannot write to it or delete it.
    ...
    //Reserved2 = 0x40,
    /// <summary>
    /// The file or directory has no other flags set.
    /// If this bit is set, all other bits in this structure MUST be clear.
    /// </summary>
    ...
    /// For a file, this means that all data in the file is compressed.
